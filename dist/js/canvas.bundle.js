...
var mousePos = { x: 20, y: 20, ativado: false };
...
Bolinha.prototype.draw = function (ctx) {
    ...
    if (this.autoAssign) {
        var vel = Math.abs(this.vel.y) + Math.abs(this.vel.x);
...
Bolinha.prototype.update = function (bolinhas) {
    ...
    if (mousePos.ativado) {
        var ang = U.arctan(this.pos, mousePos);
        var distSq = Math.pow(U.distance(this.pos, mousePos), 2);
        var oposAng = ang + Math.PI;
    ...
    bolinhas.forEach(function (bola) {
        if (bola.id !== _this.id) {
            var _ang = U.arctan(_this.pos, bola.pos);
            var _distSq = Math.pow(U.distance(_this.pos, bola.pos), 2);
            var _oposAng = _ang + Math.PI;

            //A constante 100 aqui é totalmente arbitraria
            _this.vel.x -= 100 * Math.cos(_oposAng) / (_distSq + 0.01);
            _this.vel.y -= 100 * Math.sin(_oposAng) / (_distSq + 0.01);
        }
    });
};

module.exports = { Bolinha: Bolinha, init: init };
...
var _eletron = __webpack_require__(/*! ./eletron */ "./src/js/eletron.js");

var E = _interopRequireWildcard(_eletron);
...
function animate() {
    ...
    for (var _i = 0; _i < TAM; ++_i) {
        bolinhas[_i].update(bolinhas);
    }
    for (var _i2 = 0; _i2 < TAM; ++_i2) {
        bolinhas[_i2].draw(ctx);
    }
}
animate();
...
module.exports = { randomColor: randomColor, distance: distance, random: random, arctan: arctan, add: add };
